...
void MainWindow::setupUI()
{
    ...
    weatherLayout->addWidget(m_descLabel, 1, 1);
    ...
    connect(&NetworkManager::instance(), &NetworkManager::networkError,
            [this](const QString &error) {
                ...
                if (!m_currentCityId.isEmpty()) {
                    loadCachedWeatherData(m_currentCityId);
                }
            });
...
QString MainWindow::weatherBackgroundStyle(WeatherType weatherType) const
{
    ...
    return QString(
               ...
               "QWidget#weatherPanel, QWidget#forecastPanel, QWidget#trendPanel {"
               "background-color: %1;"
               "border: 1px solid %2;"
               "border-radius: 14px;"
               "}"
...
WindDirection MainWindow::parseChineseWindDirection(const QString& windDirection) const
{
    QString dir = windDirection.trimmed();
    if (dir.contains("北") && dir.contains("东")) return WindDirection::NE;
    if (dir.contains("北") && dir.contains("西")) return WindDirection::NW;
    if (dir.contains("南") && dir.contains("东")) return WindDirection::SE;
    if (dir.contains("南") && dir.contains("西")) return WindDirection::SW;
    if (dir.contains("东")) return WindDirection::E;
    if (dir.contains("西")) return WindDirection::W;
    if (dir.contains("南")) return WindDirection::S;
    if (dir.contains("北")) return WindDirection::N;
...
WeatherModel* MainWindow::parseCurrentWeather(const QByteArray& data, const QString& cityId)
{
    ...
    if (lives.isEmpty()) {
        qWarning() << "Amap API error: missing lives field";
        return nullptr;
    }
    ...
    const double temperature = live["temperature"].toString().toDouble();
    const int humidity = live["humidity"].toString().toInt();
    const QString windDirection = live["winddirection"].toString();
    const QString windPower = live["windpower"].toString();

    weather->setTemperature(temperature);
    weather->setFeelsLike(temperature);
    weather->setHumidity(humidity);
    weather->setWindSpeed(parseWindPowerValue(windPower));
    weather->setWindDirection(parseChineseWindDirection(windDirection));
    ...
    if (!reportTime.isEmpty()) {
        weather->setUpdateTime(QDateTime::fromString(reportTime, "yyyy-MM-dd hh:mm:ss"));
    } else {
...
void MainWindow::loadCachedWeatherData(const QString& cityId)
{
    WeatherModel *cachedWeather = DataManager::instance().loadCachedWeather(cityId);
    if (cachedWeather) {
        updateWeatherDisplay(cachedWeather);
        delete cachedWeather;
    }

    ForecastModel *cachedForecast = DataManager::instance().loadCachedForecast(cityId);
    if (cachedForecast) {
        updateForecastDisplay(cachedForecast);
        updateTrendChart(cachedForecast);
